        ...
        <div class="page-content"  :class="{'page-content-closed':menushirinkStatus}">
                    ...
                    <form action="" method="" class="">
                        ...
                        <div>
                            ...
                            <div class="validitytype">
                                ...
                                <div class="explain" v-show="type.code != 3"> 建议填写优惠券 "折扣额度" 及自定义内容,描述卡券提供的具体优惠</div>
                            </div>
                        </div>
                        <div v-show="type.code != 3">
                            <span class="validity">折扣额度</span>
                            <div class="validitytype">
                                <input v-model="couponDiscount" type="number" class="quota" min="0"
                                 placeholder="" />{{couponDiscountUnit}}
                                <div class="explain" v-show="discountRatioTipShow"> 请填写0.1-9.9之间的数字，精确到小数点后一位</div>
                            </div>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <div class="validitytype">
                                ...
                                <br>
                                满 <input type="number" name="coupontext" value="" v-model="couponUseValue"  id="satisfyuse" min="0" step="1"> 元使用
                            </div>
                        </div>
                        <div>
            ...
            <div class="floating_layer" v-show="showDialogReleaseMsg">
                <div  class="layerInfo">
                    <ul class="prompt" >
                        <li>温馨提示</li>
                        <li>{{releaseStatus == 1 ? '优惠券发布成功！':'暂不能发布，请确认是否已保存?'}}</li>
                        <li ><span @click="closeDialogReleaseMsg">确认</span></li>
                    </ul>
                </div>
            </div>
            ...
            <div class="release">
              ...
              <span @click="releaseCoupon" v-text="releaseStatus == 1 ? '已发布':'发布'"></span>
            </div>
        </div>
...
export default {
    ...
    data() {
        return {
            ...
            sellerInfo: {}, // 商家基本信息
            type: {
                code: this.$route.params.coupon_type,
                title: ''
            },
            // couponType: '', // 当前优惠券类型名称
            couponName: '', //优惠券名称
            couponDiscount: '', //优惠券折扣
            couponDiscountUnit: '', //优惠券折扣单位
            couponColor: 'rgb(238, 204, 204)', // 优惠券颜色,默认色
            couponExpiry: 1, //有效期单选按钮
            ...
            couponUse: 1, // 使用条件单选按钮
            couponUseValue: '', // 使用条件类型的值
            couponGet: 2, // 来源途径单选按钮
            couponGetValue: [ {
                value: '' //来源途径类型1的值
            }, {
                value: [] // 来源途径类型2的值
            ...
            couponSend: 2, // 发放形式单选按钮
            couponMember: 0, // 会员限制单选按钮
            ...
            currentInsertId: '', // 优惠券保存后获取到的id，可用于发布操作
            saveStatus: 0, // 优惠券保存状态，默认为0，未保存
            releaseStatus: 0, // 优惠券发布状态，默认为0，未发布

            showDialogValidateMsg: false, // 优惠券内容验证的提示
            showDialogSaveMsg: false, // 优惠券保存成功的提示
            showDialogReleaseMsg: false, // 优惠券发布前的提示
    ...
    watch: {
        ...
        couponDiscount: function( val, oldVal ) {
            if ( this.type.code == 1 ) {
                if ( /^.*\..*$/g.test( this.couponDiscount ) ) {
                    this.couponDiscount = parseInt( this.couponDiscount )
                }
                if ( this.couponDiscount.toString().length > 4 ) {
                    this.couponDiscount = oldVal
                }
            ...
            if ( this.type.code == 2 ) {
                ...
                if ( this.couponDiscount.toString().length > 3 ) {
                    this.couponDiscount = oldVal
                }
            }
        },
    ...
    methods: {
        ...
        saveCoupon() {
            ...
            if ( this.type.code == 1 ) { // 代金券
                create_obj.coupon_discount_price = this.couponDiscount
            } else if ( this.type.code == 2 ) { // 折扣券
                create_obj.coupon_discount_ratio = this.couponDiscount
            }

            // 判断有效期
            if ( this.couponExpiry == 1 ) {
                create_obj.coupon_expiration_type_value = this.couponExpiryValue[ 0 ]
                // 判断是否填入了值
                ...
                create_obj.coupon_startdate_type_value = this.couponExpiryValue[ 1 ].sDate // 生效时间
                create_obj.coupon_expiration_type_value = this.couponExpiryValue[ 1 ].eDate // 到期时间
                // 判断是否填入了值
            ...
            if ( this.couponUse == 1 ) {
                ...
                create_obj.coupon_use_type_value = this.couponUseValue
                // 判断是否填入了值
            ...
            if ( this.couponGet == 1 ) {
                ...
                create_obj.coupon_get_type_value = this.couponGetValue[ 0 ].value
                // 判断是否填入了值
                ...
                create_obj.coupon_get_type_value = this.couponGetValue[ 1 ].value
                // 判断是否填入了值
            ...
            if ( this.type == '' ||
                ...
                jQuery.ajax( {
                    ...
                    success: function( res ) {
                        if ( res.erron == 0 ) {
                            $this.currentInsertId = res.insert_id
        ...
        releaseCoupon() {
            if ( this.releaseStatus == 1 ) {
                return // 如果优惠券已发布，不再做发布的操作
            }
            // 点击发布时候去请求后台保存好的数据，如果请求数据为空，说明没有保存数据成功，需要重新保存再发布
            if ( this.currentInsertId > 0 ) {
                let update_obj = {
                    coupon_status: {
                        code: 1,
                        title: '已发布'
                ...
                let $this = this
                jQuery.ajax( {
                    url: this.storageSiteUrl+'/api-json/xkhy/v2/coupon/' + this.currentInsertId,
                    type: 'PUT',
                    data: {
                    ...
                    success: function( res ) {
                        console.log( res )
                        if ( res.erron == 0 ) {
                            $this.releaseStatus = 1 // 优惠券状态为已发布
                            $this.showDialogReleaseMsg = true
                        }
                    }
                } )
            } else {
        ...
        // 弹窗提示里面的确认发布
        closeDialogReleaseMsg() {
            this.showDialogReleaseMsg = false;
            this.releaseText = '已发布';
        }
    },
    created() {
        // 判断优惠券类型并显示不同的信息
        let typeCode = this.$route.params.coupon_type;
        if ( this.type.code == 1 ) {
            this.type.title = '代金券';
            this.couponDiscountUnit = '元'
            this.coupoonMoney
        } else if ( this.type.code == 2 ) {
            this.type.title = '折扣券';
            this.couponDiscountUnit = '折'
            this.discountRatioTipShow = true //折扣券折扣额度提示信息是否显示
        } else if ( this.type.code == 3 ) {
            this.type.title = '兑换券';
    ...
    mounted() {
        ...
        jQuery( function() {
            ...
            var dateRange = new pickerDateRange( 'date_demo3', {
                ...
                minValidDate: (Date.parse( new Date() ) / 1000)-(24*3600), //最小可用时间，控制日期选择器的可选力度
                theme: 'ta',
                success: function( obj ) {
                    // $("#dCon_demo3").html('开始时间 : ' + obj.startDate + '<br/>结束时间 : ' + obj.endDate);
                    jQuery( '.date' ).html( jQuery( '#date_demo3' ).html() );
            ...
            jQuery( '#full' ).change( function() {
                ...
                jQuery( '.par' ).css( {'background-color':changeColor,'color':'#fff'} );
                jQuery( '.immediatelyuse ' ).css( {'background-color': changeColor,'color':'#fff'} );
            } )
        } );
    }
}
