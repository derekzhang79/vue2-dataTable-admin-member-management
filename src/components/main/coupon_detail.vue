<template lang="html">
    <div class="page-content-wrapper">
        <div class="page-content" :class="{'page-content-closed':menushirinkStatus}">
                        ...
                        <div class="stamp row">
                            <div class="par par_use col-xs-6">
                                <div class="stamptype">
                                    ...
                                    <div class="limit" v-show="couponUse == 2">满{{couponUseValue}}元使用</div>
                                </div>
                            </div>
                            <div class="use col-xs-6">
                                ...
                                <div class="line"></div>
                                <div class="publicnumber"  v-show="couponGet == 2">消费满{{couponGetValue[0].value}}元获得</div>
                ...
                <div class="row setting">
                    ...
                    <form action="" method="" class="">
                        <div>
                            ...
                            <span class="shoptitleInfo">{{sellerInfo.sellerName}}</span>
                        </div>
                        <div>
                            <span class="validity">折扣券标题</span>
                            <div class="validitytype">
                                <!-- <input v-model="couponName " type="text" name="" placeholder="" maxlength="14" /> -->
                                <span style="display:inline-block;line-height:30px;height:30px;border-bottom:1px solid #eee">{{couponName}}</span>
                        ...
                        <div>
                            <span class="validity">优惠券颜色</span>
                            <input id="full">
                        </div>
                        <div>
                            <span class="validity"> 有效期</span>
                            <div class="validitytype">
                              <span v-show="couponExpiry == '1'">
                        ...
                        <div>
                            ...
                            <div class="validitytype">
                                 <label class="mt-radio" v-show="couponUse == '1'">
                                    <input type="radio" name="conditionRadios"  value="1" v-model="couponUse">无门槛使用
                                    <span></span>
                                </label>
                                <label class="mt-radio" v-show="couponUse == '2'">
                                    <input type="radio" name="conditionRadios"   id="radio_threshold" value="2" v-model="couponUse">有门槛使用
                                    <span></span>
                                </label>
                                <span v-show="couponUse == '2'">满 <input type="number" name="coupontext" value="" v-model="couponUseValue"  id="satisfyuse" min="0" step="1"> 元使用</span>
                            </div>
                        </div>
                        <div>
                            <span class="validity">来源途径</span>
                            <div class="validitytype">
                              <span v-show="couponGet == '2'">
                                <label class="mt-radio" >
                        ...
                        <div>
                            ...
                            <div class="validitytype">
                                 <label class="mt-radio" v-show="couponSend == '1'">
                                    <input type="radio" name="userRadios" id="" value="1" v-model="couponSend">用户自领
                                    <span></span>
                                </label>
                                <label class="mt-radio" v-show="couponSend == '2'">
            ...
            <!-- 确认保存弹窗提示 -->
            <div class="floating_layer" v-show="dialogValidateMsgShow">
                <div class="layerInfo">
                    <ul class="prompt" >
                        <li>温馨提示</li>
            ...
            <div class="floating_layer" v-show="dialogReleaseMsgShow">
                <div class="layerInfo">
                    <ul class="prompt" >
                        <li>温馨提示</li>
                        <li>{{releaseStatus == 1 ? '优惠券发布成功！':'您确定要发布这张优惠券吗?'}}</li>
                        <li><span @click="closeDialogReleaseMsg">确认</span></li>
                    </ul>
                </div>
            </div>
            <div class="release">
              <span class="save" @click="releaseCoupon"  v-text="releaseStatus == 1 ? '已发布':'发布'"></span>
...
export default {
    ...
    data() {
        return {
            ...
            dialogValidateMsgShow: false, // 优惠券内容验证的提示
            dialogReleaseMsgShow: false, // 优惠券发布时的提示

            limitClickShow: true, // 时间选择插件限制点击浮层

            couponMemberShow: '' // 显示数据对应的会员等级限制类型
        }
    },
    ...
    methods: {
                ...
                jQuery.ajax( {
                    ...
                    success: function( res ) {
                        ...
                        if ( res.erron == 0 ) {
                            ...
                            $this.dialogReleaseMsgShow = true
                        }
                    }
                } )
            }
        ...
        closeDialogValidateMsg() {
            this.dialogValidateMsgShow = false;
        },
        // 关闭优惠券保存成功的提示框
        closeDialogReleaseMsg() {
            this.dialogReleaseMsgShow = false
        },
    },
    created() {
        let $this = this
        ...
        jQuery.ajax( {
            ...
            success: function( res ) {
                if ( res.erron == 0 ) {
                    ...
                    if ( JSON.parse( data.coupon_status ).code == 1 ) { // 判断优惠券发布状态，修改发布状态的显示
